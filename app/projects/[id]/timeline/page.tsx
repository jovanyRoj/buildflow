    ...
    <div className="pb-24 flex flex-col h-screen">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-32 flex-shrink-0 bg-white border-r border-gray-100 overflow-y-auto">
          ...
          {tasks.map(task => (
            <div key={task.id} className="h-10 flex items-center px-3 border-b border-gray-50 cursor-pointer hover:bg-gray-50"
          ...
          <div style={{ width: timelineWidth, minWidth: '100%' }}>
            ...
            <div className="relative">
              ...
              {Array.from({ length: totalDays }).map((_, i) => {
                ...
                return <div key={i} className="absolute top-0 bottom-0 bg-gray-50 pointer-events-none"
                  style={{ left: i * DAY_WIDTH, width: DAY_WIDTH }} />
              })}
              {tasks.map(task => {
                const style = getBarStyle(task)
                return (
                  <div key={task.id} className="relative h-10 border-b border-gray-50 flex items-center">
                    <div className={`absolute h-6 ${TIMELINE_BAR_COLORS[task.status]} rounded-md cursor-pointer hover:opacity-80 transition flex items-center px-2`}
                      style={{ left: style.left, width: style.width }}
                      onClick={() => router.push(`/projects/${project.id}/tasks/${task.id}`)}>
                      {style.width > 50 && <span className="text-white text-[10px] font-medium truncate">{task.name}</span>}
                    </div>
                  </div>
                )
              })}
