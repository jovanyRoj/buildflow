...
const STATUS_LABELS: Record<TaskStatus, string> = { pending: 'Pending', active: 'Active', delayed: 'Delayed', completed: 'Completed' }
...
export default function TaskDetailPage() {
  ...
  useEffect(() => {
    if (task) setForm({ status: task.status, startDate: task.startDate, endDate: task.endDate, assignedTo: task.assignedTo, notes: task.notes })
  }, [task?.id])
  ...
  const hasChanges = form.status !== task.status || form.startDate !== task.startDate ||
    form.endDate !== task.endDate || form.assignedTo !== task.assignedTo || form.notes !== task.notes
      ...
      <div className="px-4 py-4 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {STATUSES.map(s => (
              <button key={s} onClick={() => setForm(f => ({ ...f, status: s }))}
                className={`py-2.5 rounded-xl border text-sm font-medium transition ${form.status === s ? STATUS_COLORS[s] + ' border-2' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                {STATUS_LABELS[s]}
              </button>
            ))}
          </div>
        ...
        <div className="card p-4">
          ...
          <input className="w-full px-3 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm"
            placeholder="Subcontractor or trade name" value={form.assignedTo}
        ...
        <div className="card p-4">
          ...
          <textarea className="w-full px-3 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm resize-none"
            placeholder="Add notes about this task..." rows={3}
