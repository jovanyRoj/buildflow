...
export default function ProjectDetailPage() {
  ...
  const { currentUser, getProject, deleteProject } = useBuildFlowStore()
    ...
    <div className="pb-24">
      ...
      <div className="bg-[#1A2B4A] px-5 py-5">
        ...
        <div className="h-2 bg-white/20 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-700 ${project.status === 'delayed' ? 'bg-red-400' : project.status === 'completed' ? 'bg-green-400' : 'bg-blue-400'}`}
        ...
        <div className="card divide-y divide-gray-50">
          {project.tasks.map(task => (
            <Link key={task.id} href={`/projects/${project.id}/tasks/${task.id}`}>
              <div className="px-4 py-3.5 flex items-center gap-3 hover:bg-gray-50 transition active:bg-gray-100">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {task.delayDays > 0 && (
                    <span className="text-xs text-red-500 font-medium">+{task.delayDays}d</span>
                  )}
            ...
            {[...project.history].reverse().slice(0, 8).map(h => (
              <div key={h.id} className="px-4 py-3">
                ...
                {h.previousValue && h.newValue && (
                  <p className="text-xs text-gray-400 mt-0.5">
                    <span className="line-through">{h.previousValue}</span>
      ...
      {showDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-6" onClick={() => setShowDelete(false)}>
          <div className="card w-full p-6" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-500 mb-5">This action cannot be undone. All tasks and history will be permanently deleted.</p>
